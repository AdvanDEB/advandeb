...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={AdvanDEB Platform - Architecture Rationale v12.24.2},
...
\title{
    ...
    \vspace{0.3cm}
    \normalsize Version 12.24.2 - Architecture Revision December 2024
}
...
\section{ARCHITECTURE REVISION - December 2024}

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=\textbf{Important: Architecture Model Updated}]
\textbf{This document has been updated to reflect a significant architectural revision made in December 2024.} The component roles and relationships have been clarified to better support implementation and user experience.
\end{tcolorbox}

\subsection{Revised Component Roles}

The AdvanDEB Platform architecture has been revised to clarify the roles of each component:

\textbf{advandeb-modeling-assistant} is now the \textbf{Main Platform GUI} and single entry point for all users. It provides:
\begin{itemize}[noitemsep]
    \item Complete user interface (Vue.js frontend + FastAPI backend)
    \item Authentication and user management (Google OAuth)
    \item Role-based access control
    \item Chat interface, knowledge exploration, document ingestion, and modeling features
    \item Integration of all platform capabilities in one unified interface
\end{itemize}

\textbf{advandeb-knowledge-builder} is now a \textbf{Toolkit/Package} (not a standalone application). It provides:
\begin{itemize}[noitemsep]
    \item Python package with robust knowledge operations
    \item Fact extraction, stylized facts, document ingestion, graph building
    \item No user interface - pure backend logic
    \item Imported and used by Modeling Assistant backend
    \item Can be used independently for batch processing
\end{itemize}

\textbf{advandeb-MCP} is a \textbf{Tool Server} for LLM agent workflows. It provides:
\begin{itemize}[noitemsep]
    \item Model Context Protocol (MCP) server in Rust
    \item Exposes platform tools (both KB and MA operations) via MCP
    \item Internal service (no authentication) for agent-powered features
    \item Used by Modeling Assistant for chat and intelligent extraction
    \item Wraps Knowledge Builder operations as MCP tools
\end{itemize}

\subsection{Key Changes from Previous Model}

\textbf{Previous Model:}
\begin{itemize}[noitemsep]
    \item Knowledge Builder: Standalone FastAPI + Vue application
    \item Modeling Assistant: Separate FastAPI + Vue application
    \item Both had their own user interfaces and backends
\end{itemize}

\textbf{New Model:}
\begin{itemize}[noitemsep]
    \item Modeling Assistant: Main GUI with single frontend and backend
    \item Knowledge Builder: Toolkit package (no UI)
    \item MCP: Tool server exposing operations for LLM agents
\end{itemize}

\subsection{Benefits of Revised Architecture}

\textbf{Simpler User Experience:}
\begin{itemize}[noitemsep]
    \item One application to learn and use
    \item Single login with consistent interface
    \item Features shown/hidden based on user role
    \item No confusion about which component to access
\end{itemize}

\textbf{Cleaner Architecture:}
\begin{itemize}[noitemsep]
    \item Clear separation: GUI vs toolkit vs agent services
    \item Knowledge Builder becomes more reusable
    \item Easier to understand component responsibilities
    \item Better alignment with actual implementation
\end{itemize}

\textbf{Easier Development and Deployment:}
\begin{itemize}[noitemsep]
    \item Single frontend codebase to maintain
    \item KB toolkit can be versioned and tested independently
    \item One web application to deploy (MA)
    \item MCP developed in parallel without affecting GUI
\end{itemize}

\subsection{Impact on This Document}
...
\begin{itemize}[noitemsep]
    \item When discussing user interfaces, authentication, and web features: refers to \textbf{Modeling Assistant GUI}
    \item When discussing knowledge operations, fact extraction, and graph building: refers to \textbf{Knowledge Builder toolkit}
    \item When discussing agent workflows and LLM integration: refers to \textbf{MCP server}
    \item When discussing integration: MA imports KB as package, MA calls MCP for agents
\end{itemize}
...
\subsection{Integration in New Model}

\textbf{Component Relationships:}
\begin{itemize}[noitemsep]
    \item \textbf{Users} → authenticate with \textbf{Modeling Assistant GUI}
    \item \textbf{MA Backend} → imports \textbf{Knowledge Builder} as Python package
    \item \textbf{MA Backend} → calls \textbf{MCP Server} for agent features
    \item \textbf{MCP Server} → wraps \textbf{KB operations} as MCP tools
    \item All components share \textbf{MongoDB} database
\end{itemize}

\textbf{Data Flow Example - Document Upload:}
\begin{enumerate}[noitemsep]
    \item User logs into Modeling Assistant GUI
    \item User uploads document via web interface
    \item MA backend: \texttt{from advandeb\_kb import ingest\_document}
    \item KB package processes and stores in MongoDB
    \item MA returns success to frontend
\end{enumerate}

\textbf{Data Flow Example - AI Chat:}
\begin{enumerate}[noitemsep]
    \item User types question in MA chat interface
    \item MA sends request to MCP server
    \item MCP executes tools (using KB functions) and calls Ollama
    \item MCP returns formatted response to MA
    \item MA displays response to user
\end{enumerate}

For complete details on the revised architecture, see \texttt{ARCHITECTURE-REVISION.md} in the documentation repository.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Architecture Revision (December 2024)}: Clarification of component roles - MA as main GUI, KB as toolkit, MCP as tool server
    \item \textbf{Authentication \& User Management}: Google OAuth (hosted by MA), JWT tokens, capability-based roles, API keys, audit logging
    \item \textbf{Knowledge Builder Toolkit}: Fact extraction, stylized facts, document ingestion, knowledge graph building, agent workflows - provided as Python package
    \item \textbf{Modeling Assistant GUI}: Main platform interface, chat, knowledge exploration, modeling scenarios, role-based feature access
    \item \textbf{MCP Tool Server}: Model Context Protocol server exposing KB and MA operations for LLM agent workflows
    \item \textbf{Integration Architecture}: Package imports, internal service calls, shared database
...
\subsection{The Three-Component Architecture (Revised December 2024)}

\begin{rationale}{Design Decision: Modeling Assistant as Main GUI, Knowledge Builder as Toolkit, MCP as Tool Server}
...
\textbf{Integration Pattern}: The revised architecture uses simple, direct integration:
\begin{itemize}[noitemsep]
    \item MA backend imports KB package: \texttt{from advandeb\_kb import ingest\_document}
    \item MA backend calls MCP server via internal HTTP/WebSocket for agent features
    \item MCP wraps KB operations as tools for LLM agents
    \item All components share MongoDB database for data consistency
\end{itemize}
...
\subsection{The Four Capabilities}

\begin{rationale}{Design Decision: Knowledge Creation, Agent Access, Analytics Access, and Reviewer Status as separately requestable capabilities}
...
Configuration management across environments uses environment variables and configuration files, never hardcoded values. The same code artifact deploys to all three environments with different configuration. This ensures that staging tests are testing the actual code that will deploy to production, not a similar-but-different version. Configuration-as-code practices enable reproducible environment creation and disaster recovery.
